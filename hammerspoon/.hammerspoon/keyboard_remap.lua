...
function keyboardRemap.getConnectedKeyboards()
    ...
    for _, device in ipairs(hidDevices) do
        ...
        if device.usagePage == 0x01 and device.usage == 0x06 then -- Generic Desktop / Keyboard usage
            print(string.format("Found keyboard device: %s (Vendor: 0x%04x, Product: 0x%04x)", 
                device.productName or "Unknown", device.vendorID or 0, device.productID or 0))
...
function keyboardRemap.findKeyboards()
    ...
    for i, keyboard in ipairs(keyboards) do
        print(string.format("%d: %s (Vendor: %s, VendorID: %d, ProductID: %d, Transport: %s)",
            i,
            keyboard.name,
            keyboard.vendorName,
