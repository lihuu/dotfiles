...
本 skill 通过 launcher 脚本调用 CLI，自动适配多种安装方式：

```bash
RT="<skill-directory>/scripts/run-rt"
```

launcher 会依次查找：`$REMOTE_TERMINAL_BIN` → 项目 `.venv/bin/rt` → PATH 中的 `rt` → PATH 中的 `remote-terminal` → `python3 -m remote_terminal.cli`。

如果 `rt` 已通过 `pipx install -e .` 全局安装，也可以直接用 `rt`。
...
1. **连接**：`"$RT" create <host> main` — 如果远端报缺少 tmux，自动 fallback 到 ssh-direct
